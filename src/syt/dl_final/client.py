...
class LoginUI(QDialog):
    def __init__(self):
        ...
        image_label = self.findChild(QLabel, 'label_2')
        image_path = os.path.join(os.path.join(current_dir, "loginui_data/background.png"))
        image_label.setStyleSheet(f"border-image: url('{image_path}') 0 stretch;")
...
# Client UI
from_class_client = uic.loadUiType(os.path.join(current_dir, "client.ui"))[0]
    ...
    def __client_connection(self, connection, address):
        ...
        while self.__running:
            try:
                ...
                print(f"Exception occurred while processing frame: {e}")
                # 예외가 발생하면 다시 프레임을 받을 수 있도록 초기화
                data = b""
