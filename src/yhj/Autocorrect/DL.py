...
from jamos import gesture2text, cons, vowels, cons_double, cons_in_double
import pandas as pd
    ...
    def on_text_changed(self):
        ...
        if self.text.strip():  # 입력이 공백이 아닌 경우에만 처리
            if self.text[-1] == " ":
                ...
                self.prefix = j2hcj(h2j(self.prefix))
                print(self.prefix)
    
                self.prefix = gesture2text(self.prefix)
                print(self.prefix)
                self.input.setText(self.prefix)
