...
1. **네트워크 : 다중 클라이언트 제어**
  - 서버와 클라이언트의 구성을 확장성 있도록 PYTHON SOCKET.IO 기반 화상 통신 플랫폼을 구현
  - 서버 상에서 약속된 합의에 ‘선택하여’ ‘1대1 매칭 통신 가능

2. **딥러닝 요소**
  - 미디어파이프 기반 지문자 데이터 셋 수집
  - 자체 설계한 딥러닝 모델을 이용해 지문자 인식

3. **편의 기능 (자동완성 / stt / tts)**
  - 단어 및 문장을 신속하게 조합 가능해 실질적인 대화가 가능한 서비스 구현
  - 음성과 텍스트 처리를 원활하게 통합  → 효율적인 의사소통 가능
