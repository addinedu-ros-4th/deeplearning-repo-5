...
class WindowClass(QDialog):
        ...
        while True:
            ...
            for sock in read_sockets:
                if sock == self.server_socket:
                    ...
                    # 추가
                    self.AddUserLogsTable(ip, port, username, peer= '', state='ON')
                else:
                    # 클라이언트로부터 데이터 수신
                    data = sock.recv(1024)
                    if data:
                        ...
                        if id == 'Connecting':
                            ...
                            self.AddUserLogsTable(ip, port, username, message, state='Connecting')
                            ...
                            table_toggle = 2
                            self.ModifyClientFromTable(table_toggle, ip, port, message)
                            self.AddUserLogsTable(ip, port, username, message, state='Connected')
                        ...
                        # 추가
                        self.AddUserLogsTable(ip, port, username, peer= '', state='OFF')
    ...
    # 추가
    def AddUserLogsTable(self, ip, port, username, peer, state):
        # if table_toggle == 2:
